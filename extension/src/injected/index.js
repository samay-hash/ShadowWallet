// Injected Script — runs in MAIN world (page's JS context)
// Overrides ALL Solana wallet providers so dApps connect to the shadow wallet
// Supports: Phantom, Backpack, Solflare, Glow, Brave Wallet

(function () {
  'use strict';

  // ─── Async bridge to content script ─────────────────────────────────────────
  ...
  window.addEventListener('message', (event) => {
    ...
    if (!type?.endsWith('_RESPONSE') || !requestId) return;
  ...
  window.addEventListener('message', (event) => {
    if (event.data?.type === 'SW_REPUTATION_RESULT') {
      siteReputation = event.data.payload || siteReputation;
    }
  });

  // ─── ShadowWalletProvider class ──────────────────────────────────────────────
  class ShadowWalletProvider {
    constructor(originalProvider) {
      ...
      console.log('%c🛡️ ShadowWallet Active — Your main wallet is protected.',
    ...
    async connect(options) {
      try {
        ...
        if (!result?.publicKey) {
          ...
          this._publicKey = new FakePublicKey(activated?.wallet?.publicKey || 'ShadowDefault');
        } else {
    ...
    request(method, params) {
      return this._original?.request?.(method, params);
    }
  }

  // ─── Fake PublicKey (Phantom-compatible) ─────────────────────────────────────
  class FakePublicKey {
    constructor(base58) { this._base58 = base58; }
    toBase58() { return this._base58; }
    toString() { return this._base58; }
    toJSON() { return this._base58; }
    equals(other) { return this._base58 === other?.toBase58?.(); }
  }
  ...
  function createOverlay(analysis, siteUrl, resolve) {
    ...
    el.querySelector('#sw-block-btn')?.addEventListener('click', () => {
      el.remove();
      resolve(false);
    });
    el.querySelector('#sw-approve-btn')?.addEventListener('click', () => {
      el.remove();
      resolve(true);
    });
    return el;
  ...
  function updateOverlay(el, analysis) {
    ...
    if (warningsEl) {
      warningsEl.innerHTML = (analysis.warnings || []).map(w =>
        `<div style="background:rgba(255,51,102,0.1);border:1px solid rgba(255,51,102,0.3);border-radius:6px;padding:6px 10px;font-size:12px;color:#fca5a5;margin-bottom:6px">⚠️ ${w}</div>`
      ).join('');
    }
